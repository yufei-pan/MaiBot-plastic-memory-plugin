...
**自动压缩**  
演化指令超过字符上限时，会调用 LLM 按麦麦的人格与表达风格压缩重写。超限时 `append_instruction` / `rewrite_instruction` 会在后台异步触发压缩；也可以主动调用 `compact_instructions` 同步等待结果。
...
向自我演化接口写入演化指令。若结尾没有换行会自动补上。超过上限时后台异步压缩，工具立即返回。
...
用 `content` **完全覆盖**演化指令。超过上限时同样在后台异步压缩。
