...
**LLM 写入整理（默认开启）**  
`append_instruction` / `rewrite_instruction` 默认不会把 planner 原始参数原样落盘，而是（除 `insert_after_string` 外）将工具参数交给 **replyer** 模型，结合人格与表达风格整理为演化指令正文。LLM 连续返回空时会重试（默认最多 3 次），仍失败则回退为 `content` / `instruction` / `note` / `text` 等字段的直接合并结果。可通过 `llm_rewrite_writes = false` 关闭，恢复旧行为。
...
rewrite_max_tokens = 0          # 0 = max(4096, size_limit)
...
**`llm_rewrite_writes`** — `append_instruction` / `rewrite_instruction` 写入前是否经 LLM 整理。默认 `true`  
**`rewrite_model`** — 写入整理使用的 LLM 任务名。默认 `replyer`  
**`rewrite_temperature`** — 写入整理采样温度。默认 `0.3`  
**`rewrite_max_tokens`** — 写入整理 `max_tokens`；`0` 表示 `max(4096, size_limit)`。默认 `0`  
