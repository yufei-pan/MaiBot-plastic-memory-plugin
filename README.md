...
**LLM 写入整理（默认开启）**  
`append_instruction` / `rewrite_instruction` 默认不会把 planner 原始参数原样落盘，而是将实质正文（`content` / `instruction` / `note` / `text` 等）交给 **replyer** 模型，结合人格与表达风格整理为演化指令正文。Host 注入的 `stream_id` 等会话上下文字段**不会**进入整理 prompt。若无实质正文则拒绝写入（不会因上下文 kwargs 误触发 LLM）。LLM 连续返回空或失败时会重试（默认最多 3 次），仍失败则回退为字段直接合并，并在工具返回中附加「LLM 整理未成功，已使用原始参数写入」。可通过 `llm_rewrite_writes = false` 关闭，恢复升级前直写行为。
