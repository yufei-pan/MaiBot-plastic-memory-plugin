...
class PlasticMemoryPlugin(MaiBotPlugin):
    ...
    async def _compact(self) -> int:
        ...
        async with self._store.lock:
            ...
            max_tokens = _resolve_compact_max_tokens(self._compact_max_tokens, self._size_limit)
            if self._compact_max_tokens > 0 and max_tokens < self._size_limit:
            ...
            for attempt in range(1, self._max_compact_attempts + 1):
                ...
                result = await self._generate_compact_llm(prompt, max_tokens, self._size_limit)
                if not result.get("success"):
                    self.ctx.logger.warning(
